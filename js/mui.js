...
(function ($, window, undefined) {
    ...
    var ajaxComplete = function (status, xhr, settings) {
        if (xhr.status == 600) {//Session过期
            window.top.location.replace("/?t="+ new Date().getTime());
        ...
        } else if (xhr.status >= 0 && settings.comet != true) { //服务器忙
            if (!settings.error) {
                window.top.location.href = window.top.root + "/errors/" + xhr.status + ".html";
            }
    ...
    $.toast = function (message, callback) {
        if ($.os.plus) {
            ...
            setTimeout(function () {
                ...
                if (callback && callback.constructor && (callback.constructor == Function || (callback.constructor.name && callback.constructor.name == "Function"))) {
                    callback.apply();
                }
            }, 2000);
        }
...
(function ($) {
     ...
     * @returns {*}
     */
    $.cookie = function (sKey, sValue, vEnd, sPath, sDomain, bSecure) {
        if (!sValue) {
            /*set cookie*/
